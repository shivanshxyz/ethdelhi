...
contract MEVHookTest is Test, Deployers {
    ...
    function test_applyRecommendation_validSignature_applies_override() public {
        address syntheticPool = address(uint160(uint256(PoolId.unwrap(poolId))));
        vm.prank(hook.owner());
        hook.addAuthorizedOracle(address(0xA11CE));

        // Sign using a private key corresponding to 0xA11CE
        uint256 pk = 0xA11CE;
        address signer = vm.addr(pk);
        vm.prank(hook.owner());
        hook.addAuthorizedOracle(signer);

        uint16 fee = 75; // 0.75%
        uint256 deadline = block.timestamp + 60;
        uint256 nonce = hook.nonces(signer);
        ...
        // Domain separator computed as in contract
        ...
        hook.applyRecommendation(syntheticPool, fee, deadline, sig, metadataHash, nonce);
    ...
    function test_applyRecommendation_replay_or_expired_reverts() public {
        address syntheticPool = address(uint160(uint256(PoolId.unwrap(poolId))));
        uint256 pk = 0xB0B;
        address signer = vm.addr(pk);
        vm.prank(hook.owner());
        ...
        uint256 nonce = hook.nonces(signer);
        bytes32 metadataHash = keccak256("meta2");
        ...
        // Expired should revert
        uint256 nonce2 = hook.nonces(signer);
        ...
        bytes32 digest2 = keccak256(abi.encodePacked("\x19\x01", ds, structHash2));
        (v, r, s) = vm.sign(pk, digest2);
        bytes memory sig2 = abi.encodePacked(r, s, v);
        vm.expectRevert("EXPIRED");
        hook.applyRecommendation(syntheticPool, fee, pastDeadline, sig2, metadataHash, nonce2);
    }
}
